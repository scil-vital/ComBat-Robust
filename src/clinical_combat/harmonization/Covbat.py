# -*- coding: utf-8 -*-
"""CovBat harmonization method built on top of CombatPairwise."""
import logging
...
class CovBat(CombatPairwise):
    """
    CovBat implementation following Chen et al. (2021).
    ...
    def __init__(
        ...
        """
        covbat_pve: float
            Proportion of variance explained threshold used to select the number
            of principal components in the residual space.
        covbat_max_components: int | None
            Optional cap on the number of components.

        The following parameters are inherited from CombatPairwise:
        - alpha, beta, sigma
        - gamma_ref, delta_ref, gamma_mov, delta_mov
        """
        super().__init__(
    ...
    def initialize_from_model_params(self, model_filename):
        ...
        self.covbat_max_components = covbat_params.get("max_components", self.covbat_max_components)
        self.covbat_feature_mean = np.array(
            covbat_params.get("feature_mean", []), dtype=float
        )
        self.covbat_components = np.array(
            covbat_params.get("components", []), dtype=float
        )
    ...
    def set_model_fit_params(self, ref_data, mov_data):
        ...
        self.model_params["covbat"] = {
            "pve": float(self.covbat_pve),
            "max_components": self.covbat_max_components,
        }

    def fit(self, ref_data, mov_data):
        """
        CovBat fit. First runs classic ComBat, then estimates a PC basis in the
        residual space to align covariance.
        ...
        if combined_residual.size == 0:
            logging.warning(
                "CovBat skipped due to empty residual matrix; falling back to classic ComBat."
            )
        ...
        if centered.shape[0] <= 1:
            logging.warning(
                "CovBat skipped because covariance cannot be estimated (<=1 subject)."
            )
            self.covbat_components = np.zeros((len(self.bundle_names), 0))
        ...
        if q <= 0:
            logging.info(
                "CovBat threshold yielded zero components; falling back to classic ComBat."
            )
            self.covbat_components = np.zeros((len(self.bundle_names), 0))
            self.covbat_feature_mean = feature_mean
            self.covbat_site_means = {ref_site: np.array([]), mov_site: np.array([])}
    ...
    def apply(self, data):
        ...
        site_kind = "ref" if site == self.covbat_target_site else "mov"
        residual, expected = self._compute_residual_and_expected(design, Y, site_kind)
    ...
    def _compute_residual_and_expected(self, design, Y, site):
        """
        Compute the residual matrix and the expected value matrix, in the same
        scale used by CovBat.

        design: list[array]
            Design matrices per bundle.
        Y: list[array]
            Observations per bundle.
        site: str
            "ref" or "mov" indicating which site parameters to use.

        Returns
        -------
        residual: array
            Shape (n_subjects, n_bundles).
        expected: array
            Shape (n_subjects, n_bundles).
        """
        ...
        residual = np.zeros((n_subjects, n_bundles), dtype=float)
        expected = np.zeros((n_subjects, n_bundles), dtype=float)

        for i in range(n_bundles):
            if design[i].shape[0] > 1:
                cov_effect = np.dot(design[i][1:, :].transpose(), self.beta[i])
            else:
                cov_effect = np.zeros(n_subjects, dtype=float)

            z = (np.asarray(Y[i], dtype=float) - self.alpha[i] - cov_effect) / self.sigma[i]
    ...
    def _safe_std(scores: np.ndarray) -> np.ndarray:
        """
        Compute a safe standard deviation on component scores.

        scores: array
            PC scores for a site, shape (n_subjects, q).

        Returns
        -------
        std: array
            Standard deviation per component, with non-positive values replaced by 1.0.
        """
        if scores.size == 0:
            return np.array([])
